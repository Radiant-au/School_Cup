# Standalone Project Specification

## Purpose

Defines that the project is a self-contained React + Vite + TypeScript app: dependencies resolve from the public npm registry with no workspace/catalog protocols, TypeScript config is self-contained, the production build runs without parent-workspace files, the dev server defaults its env vars, and the Node engine requirement is declared.
